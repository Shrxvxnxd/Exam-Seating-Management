...
const { Exam, Seating, Admin, Student, Malpractice, SystemEvent, Faculty, FacultyAssignment, Attendance, syncDB } = require('./models');
...
const app = express();
app.set('trust proxy', 1); // essential for correct protocol detection behind proxies (Render/Heroku)


// Multer for Memory Storage
const upload = multer({ storage: multer.memoryStorage() });

// --- LOGGER IMPLEMENTATION FOR SYSTEM HEALTH DASHBOARD V4 ---
const fs = require('fs');
const path = require('path');
const logStream = fs.createWriteStream(path.join(__dirname, 'backend_error.log'), { flags: 'a' });

// Capture original console methods
const originalConsoleLog = console.log;
const originalConsoleError = console.error;

// Overlay console methods to write to file
...
const { trafficControl, getActiveRequests } = require('./middleware/trafficControl');
...
app.use(trafficControl); // Active Request Limiting (503 if > 500)
...
app.get('/api/auth/microsoft', authLimiter, passport.authenticate('microsoft', { prompt: 'select_account' }));
...
app.get('/api/health', async (req, res) => {
    try {
        ...
        const healthData = {
            ...
            microsoft: {
                status: (process.env.MICROSOFT_CLIENT_ID && process.env.MICROSOFT_CLIENT_SECRET) ? 'configured' : 'missing_config',
                message: (process.env.MICROSOFT_CLIENT_ID && process.env.MICROSOFT_CLIENT_SECRET) ? 'Ready for requests' : 'Missing Client ID/Secret'
            },
            system: {
...
// Logs Endpoint for System Health V4
app.get('/api/logs', async (req, res) => {
    try {
        const fs = require('fs');
        const path = require('path');
        const logPath = path.join(__dirname, 'backend_error.log'); // Adjust if logs are elsewhere, e.g. 'logs/error.log'

        if (!fs.existsSync(logPath)) {
            return res.json({ logs: ['[System] Log file not found. Waiting for events...'] });
        }

        // Read last 2KB to avoid reading huge files
        const stats = fs.statSync(logPath);
        const size = stats.size;
        const readSize = Math.min(size, 1024 * 10); // Read last 10KB
        ...
        const lines = data.split('\n').filter(line => line.trim()).slice(-50); // Get last 50 lines

        res.json({ logs: lines });
    } catch (error) {
        res.status(500).json({ logs: [`[System] Error reading logs: ${error.message}`] });
    }
});


// --- System Events Endpoint for Student Status Page ---
// --- System Events Endpoint for Student Status Page ---
app.get('/api/system-events', async (req, res) => {
    try {
        // SEEDING: If no events exist, create some samples for demo purposes
        const count = await SystemEvent.count();
        if (count === 0) {
            await SystemEvent.bulkCreate([
                {
                    type: 'maintenance',
                    title: 'System Maintenance',
                    description: 'Routine maintenance and performance upgrades.',
                    scheduled_start: new Date(new Date().getTime() + 604800000), // Next week
                    scheduled_end: new Date(new Date().getTime() + 608400000),
                    status: 'Scheduled'
                },
                {
                    type: 'incident',
                    title: 'Database Sync Improved',
                    description: 'System-wide performance patch applied.',
                    status: 'Resolved',
                ...
                },
                {
                    ...
                    description: 'All exam services are running at peak performance.',
                    status: 'Verified',
                    createdAt: new Date(new Date().getTime() - 86400000) // Yesterday
                }
            ]);
        }
        ...
        // Priority 1: Check for Active / In Progress Maintenance
        let maintenance = await SystemEvent.findOne({
            where: {
                type: 'maintenance',
                status: 'In Progress'
            }
        ...
        // Priority 2: Check for a currently active scheduled window
        if (!maintenance) {
            maintenance = await SystemEvent.findOne({
                where: {
                    type: 'maintenance',
                    status: 'Scheduled',
                    scheduled_start: { [Op.lte]: now },
                    scheduled_end: { [Op.gte]: now }
                }
            });
        }

        // Priority 3: If no active window, get the next upcoming one
        if (!maintenance) {
            maintenance = await SystemEvent.findOne({
                where: {
                    type: 'maintenance',
                    scheduled_start: { [Op.gt]: now }
                },
                order: [['scheduled_start', 'ASC']]
            });
        }

        const recentActivity = await SystemEvent.findAll({
            where: {
                type: { [Op.ne]: 'maintenance' }
            },
            order: [['createdAt', 'DESC']],
            limit: 5
        });

        res.json({ maintenance, recentActivity });
    } catch (err) {
...
// Admin ONLY: Create System Event
app.post('/api/system-events', async (req, res) => {
    try {
        // TODO: Add proper Admin Auth check (assuming simplified for now or rely on frontend guard)
        const event = await SystemEvent.create(req.body);
...
// Admin ONLY: Delete System Event
app.delete('/api/system-events/:id', async (req, res) => {
    try {
        await SystemEvent.destroy({ where: { id: req.params.id } });
        lastCacheUpdate = 0; // Invalidate cache
...
const getMaintenanceStatus = async () => {
    const now = Date.now();
    if (maintenanceCache && (now - lastCacheUpdate < CACHE_TTL)) {
        return maintenanceCache;
    }

    try {
        const currentTime = new Date();
        const active = await SystemEvent.findOne({
            where: {
                type: 'maintenance',
                [Op.or]: [
                    { status: 'In Progress' },
                    {
                        status: 'Scheduled',
                        scheduled_start: { [Op.lte]: currentTime },
                        scheduled_end: { [Op.gte]: currentTime }
                    }
                ]
            }
        });

        maintenanceCache = active;
        lastCacheUpdate = now;
        return active;
    } catch (err) {
        console.error("[Maintenance Cache Update Error]", err);
        return maintenanceCache; // Return stale cache on error
    }
};

const checkMaintenanceMode = async (req, res, next) => {
    try {
        // Whitelist: Always allow these routes
        if (req.path.startsWith('/api/admin') ||
            req.path.startsWith('/api/auth/admin') ||
            req.path === '/api/health' ||
            req.path === '/api/system-events' ||
            req.path === '/api/logs') {
            return next();
        }

        const activeMaintenance = await getMaintenanceStatus();

        if (activeMaintenance) {

            return res.status(503).json({
                error: 'System Under Maintenance',
                message: activeMaintenance.description || 'The system is currently undergoing maintenance.'
            });
        }
...
const requireAnyAdmin = (req, res, next) => {
    ...
    if (!user) return res.status(401).json({ error: 'Unauthorized' });
    if (['main_admin', 'sub_admin', 'view_only'].includes(user.role)) return next();
    return res.status(403).json({ error: `Admin Access Required. Got: ${user.role}` });
};
...
app.post('/api/auth/send-verify-email', authenticateToken, async (req, res) => {
    try {
        ...
        const mailOptions = {
            ...
            to: email,
            subject: 'Verify your Email - Vignan Student Portal',
...
app.post('/api/auth/student/otp-login-init', otpLimiter, async (req, res) => {
    try {
        const { email } = req.body;
        if (!email) return res.status(400).json({ error: 'Email required' });
        ...
        const mailOptions = {
            from: process.env.EMAIL_USER,
            to: email,
            subject: 'Your Login Code - Vignan Student Portal',
...
// Get Room Layout & Seating (Public / Restricted)
// Used by Student Theater View
app.get('/api/exams/:id/rooms/:roomName/layout', async (req, res) => {
    try {
        ...
        if (typeof exam.room_config === 'string') { try { exam.room_config = JSON.parse(exam.room_config); } catch (e) { exam.room_config = []; } }
        if (!Array.isArray(exam.room_config)) exam.room_config = [];

        if (typeof exam.batches === 'string') { try { exam.batches = JSON.parse(exam.batches); } catch (e) { exam.batches = []; } }
        if (!Array.isArray(exam.batches)) exam.batches = [];

        // Find Room Config
        // Decode URI component just in case, though Express usually handles it
        const decodedName = decodeURIComponent(roomName);
        const room = exam.room_config.find(r => r.name === decodedName);

        if (!room) return res.status(404).json({ error: 'Room not found in this exam' });

        // Fetch Occupied Seats
        const seats = await Seating.findAll({
            where: {
                ExamId: id,
                room_name: decodedName
            },
            attributes: ['seat_number', 'student_reg', 'row', 'col']
        });
...
app.get('/api/exams/:id/pdf/seating', authenticateToken, checkPermission('download_seating'), async (req, res) => {
    try {
        ...
        if (typeof exam.subjects === 'string') { try { exam.subjects = JSON.parse(exam.subjects); } catch (e) { exam.subjects = []; } }
...
const verifyTurnstile = (req, res, next) => {
    ...
    // Bypass if needed (or handle strictness)
    if (!token) {
        return res.status(400).json({ error: 'Security verification failed. Token missing.' });
    }
    ...
    const request = https.request(verificationUrl, {
        method: 'POST',
        headers: {
        ...
        let data = '';
        turnstileRes.on('data', chunk => data += chunk);
        turnstileRes.on('end', () => {
            try {
                ...
                if (json.success) {
                    ...
                    console.error("[Turnstile Debug] Verification FAILED. Error Codes:", json['error-codes']);
                    return res.status(400).json({
                        error: 'Security check failed.',
                        details: json['error-codes'] // Send details to frontend for debugging
                    });
                }
            } catch (e) {
    ...
    request.on('error', (e) => {
        console.error("Turnstile Connection Error:", e);
        return res.status(500).json({ error: 'Unable to verify security token.' });
    });
...
const authenticateFaculty = (req, res, next) => {
    ...
    jwt.verify(token, process.env.JWT_SECRET, async (err, decoded) => {
        if (err) return res.status(403).json({ error: 'Invalid Token' });
        if (decoded.role !== 'faculty') return res.status(403).json({ error: 'Faculty Access Only' });

        const faculty = await Faculty.findByPk(decoded.id);
        if (!faculty) return res.status(401).json({ error: 'User not found' });
...
// 1. Create Faculty
app.post('/api/admin/faculty', authenticateToken, requireAdminManagement, async (req, res) => {
    try {
        const { username, password, full_name, email, department, employee_id } = req.body;

        // Basic Check
        if (!username || !password || !employee_id) return res.status(400).json({ error: 'Missing Required Fields' });
        ...
        res.json({ message: 'Faculty Created', facultyId: faculty.id });
    } catch (err) {
...
// 2. Assign Class/Batch to Faculty
app.post('/api/admin/faculty/assign', authenticateToken, requireAdminManagement, async (req, res) => {
    try {

        const { faculty_id, branch, section, year, subject, start_reg, end_reg, excluded_reg, period, room_no, day_of_week } = req.body;

        if (!faculty_id || !branch || !year) return res.status(400).json({ error: 'Missing core assignment details' });
...
// 3. List All Faculty
app.get('/api/admin/faculty', authenticateToken, requireAnyAdmin, async (req, res) => {
    try {
        const list = await Faculty.findAll({
            include: [FacultyAssignment],
            order: [['createdAt', 'DESC']]
        });
        res.json(list);
    } catch (err) {
...
// 4. Edit Faculty
app.put('/api/admin/faculty/:id', authenticateToken, requireAdminManagement, async (req, res) => {
    try {
        const { username, full_name, email, department, employee_id, password } = req.body;
        const faculty = await Faculty.findByPk(req.params.id);
        if (!faculty) return res.status(404).json({ error: 'Faculty not found' });

        faculty.username = username || faculty.username;
        faculty.full_name = full_name || faculty.full_name;
        faculty.email = email || faculty.email;
        faculty.department = department || faculty.department;
        faculty.employee_id = employee_id || faculty.employee_id;
...
// 5. Delete Faculty
app.delete('/api/admin/faculty/:id', authenticateToken, requireAdminManagement, async (req, res) => {
    try {
        // First delete assignments and attendance?
        // Cascade should handle it if defined, but safe manual deletion:
        await Attendance.destroy({ where: { faculty_id: req.params.id } });
        await FacultyAssignment.destroy({ where: { faculty_id: req.params.id } });
...
// 6. Edit Assignment
app.put('/api/admin/faculty/assignment/:id', authenticateToken, requireAdminManagement, async (req, res) => {
    try {

        const { branch, section, year, subject, start_reg, end_reg, excluded_reg, period, room_no, day_of_week } = req.body;
        const assignment = await FacultyAssignment.findByPk(req.params.id);
        ...
        assignment.start_reg = start_reg || assignment.start_reg;
        assignment.end_reg = end_reg || assignment.end_reg;
        assignment.excluded_reg = excluded_reg || assignment.excluded_reg;
        assignment.period = period !== undefined ? period : assignment.period;
        assignment.room_no = room_no !== undefined ? room_no : assignment.room_no;
        assignment.day_of_week = day_of_week !== undefined ? day_of_week : assignment.day_of_week;
...
// 7. Delete Assignment
app.delete('/api/admin/faculty/assignment/:id', authenticateToken, requireAdminManagement, async (req, res) => {
    try {
        await Attendance.destroy({ where: { assignment_id: req.params.id } }); // Clear attendance for this assignment
        const deleted = await FacultyAssignment.destroy({ where: { id: req.params.id } });
...
app.post('/api/auth/faculty/login', async (req, res) => {
    try {
        ...
        const token = jwt.sign(
            { id: faculty.id, role: 'faculty', username: faculty.username },
            process.env.JWT_SECRET,
            { expiresIn: '12h' }
        );

        res.json({ token, role: 'faculty', username: faculty.username, full_name: faculty.full_name });
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
});
...
app.get('/api/faculty/assignments', authenticateFaculty, async (req, res) => {
    try {
...
// 3. Get Student List for Assignment (Dynamic)
// This is to "Fetch" students based on the assignment criteria so the faculty can mark them.
app.post('/api/faculty/fetch-students', authenticateFaculty, async (req, res) => {
    try {
        const { assignment_id } = req.body;
        const assignment = await FacultyAssignment.findOne({ where: { id: assignment_id, faculty_id: req.user.id } });

        if (!assignment) return res.status(404).json({ error: 'Assignment not found' });

        // Logic to find students based on assignment
        const { branch, section, year, start_reg, end_reg, excluded_reg } = assignment;

        // Logic: 
        // If Reg Range is provided, we should primarily trust that range. 
        // However, standard practice is usually filter by Branch/Year AND Range.
        // But users often face issues if "Year 3" != "3rd Year".
        // FIX: If ranges are set, prioritize Range search (perhaps loosely filter branch if needed, but let's trust the admin's range).

        let whereClause = {};

        // If NO range is provided, we rely strictly on Branch/Year/Section
        // If range IS provided, we can relax Year/Section constraints if they cause issues, 
        // OR we can keep them. Given user complaint "No students found", let's try to be broader.

        if (start_reg && end_reg) {
            // trust the range more. Maybe still filter by branch?
            // Let's keep branch but remove section/year strictness if range exists?
            // Or better: Just fetch all and filter by range.
            if (branch) whereClause.branch = branch;
        } else {
            // No range, use strict filters
            if (branch) whereClause.branch = branch;
            if (section && section !== 'All') whereClause.section = section;
            if (year) whereClause.current_year = { [Op.like]: `${year}%` };
        }

        // Fetch all potential students from DB
        const dbStudents = await Student.findAll({
            where: whereClause,
            attributes: ['id', 'reg_no', 'full_name', 'current_year', 'branch', 'section'],
            order: [['reg_no', 'ASC']]
        });

        let finalStudents = [];

        // If Reg Range is provided, GENERATE the complete list
        if (start_reg && end_reg) {
            const startCheck = start_reg.match(/^(.*?)(\d+)$/);
            const endCheck = end_reg.match(/^(.*?)(\d+)$/);

            if (startCheck && endCheck) {
                const prefix = startCheck[1]; // e.g. "211FA04"
                const startNumStr = startCheck[2]; // e.g. "001"
                const endNumStr = endCheck[2]; // e.g. "060"

                const startNum = parseInt(startNumStr, 10);
                const endNum = parseInt(endNumStr, 10);
                const padLen = startNumStr.length;

                // Create a map for fast lookup of EXISTING students
                const studentMap = new Map();
                dbStudents.forEach(s => {
                    if (s.reg_no) studentMap.set(s.reg_no.toUpperCase(), s);
                });

                // Create Exclusions Set (Robustly)
                let exSet = new Set();
                try {
                    let exArr = excluded_reg;
                    if (typeof exArr === 'string' && exArr.trim()) {
                        if (exArr.startsWith('[')) exArr = JSON.parse(exArr);
                        else exArr = exArr.split(',').map(e => e.trim());
                    }
                    if (Array.isArray(exArr)) {
                        exArr.forEach(e => {
                            if (e) exSet.add(String(e).toUpperCase().trim());
                        });
                    }
                } catch (e) {
                    console.error('[Exclusion Parse Error]', e);
                }

                // Generate every Reg No in the range
                for (let i = startNum; i <= endNum; i++) {
                    const numPart = String(i).padStart(padLen, '0');
                    const genReg = `${prefix}${numPart}`;

                    // Check Exclusions first
                    if (exSet.has(genReg.toUpperCase())) continue;

                    if (studentMap.has(genReg.toUpperCase())) {
                        // Case 1: Student exists in DB -> Use DB Record
                        finalStudents.push(studentMap.get(genReg.toUpperCase()));
                    } else {
                        // Case 2: Student NOT in DB -> Create Placeholder
                        finalStudents.push({
                            id: `temp_${genReg}`,
                            reg_no: genReg,
                ...
                // Formatting error in range -> Fallback to DB results only
                finalStudents = dbStudents;
            }
        } else {
            // No range provided -> Use DB results only
            finalStudents = dbStudents;
        }

        // Final safety cleanup: filter by exclusions if we fell back to dbStudents or just to be extra safe
        try {
            let exSet = new Set();
            let exArr = excluded_reg;
            if (typeof exArr === 'string' && exArr.trim()) {
            ...
            if (exSet.size > 0) {
                finalStudents = finalStudents.filter(s => s.reg_no && !exSet.has(String(s.reg_no).toUpperCase().trim()));
            }
        } catch (e) {
            console.error('[Exclusion Final Cleanup Error]', e);
        }
...
// 4. Submit Attendance
app.post('/api/faculty/attendance', authenticateFaculty, async (req, res) => {
    try {
        const { assignment_id, date, records, period, teaching_method, topic } = req.body;
        // records: [{ student_reg: '...', status: '...', remarks: '...' }]
        ...
        const payload = records.map(r => ({
            ...
            teaching_method: teaching_method || null,
            topic: topic || null
        }));

        // Delete existing for this assignment + date + PERIOD to allow updates/corrections
...
// 5. Get Attendance History (Grouped by Session)
app.get('/api/faculty/history', authenticateFaculty, async (req, res) => {
    try {
        const { date } = req.query;
        // console.log('DEBUG: Fetching history for faculty:', req.user.id, 'Date:', date); // Debug log
        const whereClause = { faculty_id: req.user.id };
        if (date) whereClause.date = date;

        // Check if raw records exist first
        /*
        const rawCheck = await Attendance.count({ where: whereClause });
        ...
        */
        ...
        const history = await Attendance.findAll({
            ...
            attributes: [
                ...
                'subject',
                'period',
                [sequelize.fn('MAX', sequelize.col('FacultyAssignment.branch')), 'branch'],
                [sequelize.fn('MAX', sequelize.col('FacultyAssignment.section')), 'section'],
                [sequelize.fn('MAX', sequelize.col('Attendance.createdAt')), 'submission_time'],
                [sequelize.fn('COUNT', sequelize.col('Attendance.id')), 'total_students'],
                [sequelize.fn('SUM', sequelize.literal("CASE WHEN Attendance.status = 'Absent' THEN 1 ELSE 0 END")), 'absent_count']
            ],
            include: [{
            ...
            group: ['date', 'Attendance.subject', 'period'],
            order: [['date', 'DESC'], ['period', 'ASC']]
        });

        // DEBUG: Log the first record to see what's actually returned
        if (history.length > 0) {
        ...
        console.error('HISTORY_ERROR:', err); // ENABLED LOG
        res.status(500).json({ error: err.message });
    }
});

// 6. Get Detailed Report for a Specific Session
app.get('/api/faculty/history/details', authenticateFaculty, async (req, res) => {
    try {
        const { date, subject, period } = req.query;
        if (!date || !subject || !period) return res.status(400).json({ error: 'Missing parameters' });
...
// 1. List Students (with search and pagination)
app.get('/api/admin/students', authenticateToken, requireAnyAdmin, async (req, res) => {
    try {
...
app.put('/api/admin/students/:id', authenticateToken, requireAdminManagement, async (req, res) => {
    try {
        const { id } = req.params;
        const { full_name, current_year, branch, section, academic_start, academic_end } = req.body;
        ...
        if (academic_start !== undefined) student.academic_start = parseInt(academic_start);
        if (academic_end !== undefined) student.academic_end = parseInt(academic_end);
...
app.get('/api/health', async (req, res) => {
    const health = {
        ...
        counts: {
            ...
            // Estimates for "Active" could be added here if we had `last_active`
        },
        system: {
            memory: process.memoryUsage(),
            os: {
    ...
    try {
        ...
        health.database.message = 'Connection Successful';
    } catch (error) {
    ...
    // 2. Check Microsoft Auth Config
    if (process.env.MICROSOFT_CLIENT_ID && process.env.MICROSOFT_CALLBACK_URL) {
        health.microsoft.status = 'configured';
        health.microsoft.message = 'Client ID & Callback URL Present';
    } else {
        health.microsoft.status = 'missing_config';
        health.microsoft.message = 'Missing Env Variables';
        health.message = 'System Degraded';
    }

    // Set Status Code based on critical failures
