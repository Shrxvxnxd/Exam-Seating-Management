...
function StudentContent() {
    ...
    // Auto-search logic disabled to prevent "Security Check Failed" errors before specific user interaction
    /*
    useEffect(() => {
        const canSearch = user?.is_profile_complete &&
            formData.regNo &&
            formData.branch &&
            formData.year &&
            !results &&
    ...
    */
    ...
    useEffect(() => {
        fetch(`${API_BASE_URL}/api/branches`)
            ...
            .then(data => {
                if (!data) return;
                // Determine if data is an array (normal) or object (error/maintenance)
                const safeData = Array.isArray(data) ? data : [];
                const unique = Array.from(new Set([...['CSE', 'ECE', 'MECH', 'CIVIL', 'IT', 'AI&DS', 'EEE', 'BIOTECH', 'CHEM'], ...safeData]));
                setBranchOptions(unique.sort());
            })
            .catch(err => {
                console.error("Failed to fetch branches", err);
                // Fallback to defaults
                setBranchOptions(['AI&DS', 'BIOTECH', 'CHEM', 'CIVIL', 'CSE', 'ECE', 'EEE', 'IT', 'MECH']);
            });
    }, []);
    ...
    const fetchSeating = useCallback(async (isPolling = false, token = null) => {
        ...
        try {
            ...
            if (formData.regNo) {
                ...
                url = `${API_BASE_URL}/api/seating?${query}`;
            } else {
                const query = new URLSearchParams({
                    branch: formData.branch,
                    year: formData.year,
                ...
                url = `${API_BASE_URL}/api/seating/summary?${query}`;
                isSummaryMode = true;
            }
            ...
            if (res.status === 503) {
                ...
                setMaintenanceInfo({
                    message: maintenanceData.error || 'System Under Maintenance',
            ...
            if (res.ok && data.length > 0) {
                ...
                if (!isPolling) {
                    const errorMsg = data.error || (isSummaryMode ? 'No exams found for this branch.' : 'No allocation found. Please verify your registration number & try again.');
                    // Append details if available (for debugging Turnstile)
                    const finalError = data.details ? `${errorMsg} (Code: ${data.details})` : errorMsg;
                    setError(finalError);
                }
            }
        } catch (err) {
    ...
    }, [formData.regNo, formData.branch, formData.year, examParam, API_BASE_URL]);

    const handleSearch = (e) => {
        if (e) e.preventDefault();

        if (!formData.regNo || !formData.branch || !formData.year) {
            setError("Please enter Registration Number, Branch, and Year.");
            return;
        }

        // Direct Search Logic (Captcha Removed)
        fetchSeating(false);
    };
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => {
            fetchSeating(true);
        }, 5000); // Polling for updates every 5s (Stable Fast!)

        return () => clearInterval(interval);
    }, [results, isSummary, formData.regNo, formData.branch, formData.year, examParam, fetchSeating]);
    */
    ...
    useEffect(() => {
        ...
        const recover = async () => {
            try {
                const res = await fetch(`${API_BASE_URL}/api/branches`, { cache: 'no-store' });
                if (res.status === 200) {
                    setIsMaintenance(false);
                }
            } catch (err) {
                // Silently ignore failures during recovery check
            }
        };

        const interval = setInterval(recover, 5000); // Check every 5s (Stable Fast!)
        return () => clearInterval(interval);
    }, [isMaintenance]);

    // Detect lockdown start even if not searched yet
    useEffect(() => {
        if (isMaintenance) return;
        ...
        const interval = setInterval(checkLockdown, 5000); // Check every 5s (Stable Fast!)
        return () => clearInterval(interval);
    }, [isMaintenance]);
    ...
    return (
        <div className="min-h-screen font-sans selection:bg-slate-900 selection:text-white bg-slate-50 flex flex-col">
            ...
            <div className="bg-white border-b border-slate-200">
                <div className="container mx-auto px-6 py-12 max-w-6xl">
                    <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
                        <div>
                            <div className="inline-flex items-center gap-2 px-3 py-1 bg-amber-50 border border-amber-100 rounded-full text-amber-700 text-[10px] font-black uppercase tracking-widest mb-4">
                                <Sparkles size={12} />
                                <span>Student Portal Smart Management</span>
                            </div>
                            <div className="flex items-center gap-3">
                                <h1 className="text-3xl md:text-4xl font-black text-slate-900 tracking-tight">
                                    {isMounted ? greeting : 'Hello'}, <span className="text-indigo-600">{user.name?.split(' ')[0]}</span>
                                </h1>
                                {isMounted && currentTime && (
                                    <div className="hidden sm:flex items-center gap-1.5 px-3 py-1.5 bg-white shadow-sm border border-slate-200 rounded-2xl text-slate-500 font-bold text-sm">
                            ...
                            <p className="text-slate-500 font-medium mt-2">
                                Welcome to your exam dashboard. Access your schedules and hall tickets.
                            </p>
                        </div>
                        <div className="hidden md:block text-right">
                            <div className="text-xs font-bold text-slate-400 uppercase tracking-widest mb-1">Registration Number</div>
                            <div className="text-xl font-black text-slate-800 font-mono bg-slate-100 px-4 py-2 rounded-xl border border-slate-200">{user.reg_no}</div>
                        </div>
                    </div>
            ...
            <div className="container mx-auto px-6 max-w-6xl py-12 space-y-16">
                ...
                {/* --- Search / Room Allotment Section --- */}
                <div id="room-allotment" className="scroll-mt-32">
                    <div className="flex items-center gap-3 mb-8">
                        <div className="w-10 h-10 rounded-xl bg-indigo-50 flex items-center justify-center text-indigo-600">
                            <Search size={20} />
                        </div>
                        <h2 className="text-2xl font-black text-slate-900 tracking-tight">Find Your Seat</h2>
                    </div>

                    <div className="bg-white rounded-[2rem] shadow-sm border border-slate-200 p-3 ring-1 ring-slate-200/50">
                        <form onSubmit={handleSearch} className="flex flex-col lg:flex-row gap-3">
                            <div className="flex-1 grid grid-cols-1 md:grid-cols-3 gap-3">
                                ...
                                <div className="relative group">
                                    ...
                                    <input
                                        ...
                                        className="w-full pl-12 pr-4 py-4 bg-slate-50/50 hover:bg-white border border-slate-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-bold text-slate-800 placeholder:text-slate-400 outline-none uppercase text-sm tracking-wide shadow-sm focus:shadow-md"
                                ...
                                <div className="relative group">
                                    ...
                                    <select
                                        ...
                                        onChange={handleChange}
                                        className="w-full pl-12 pr-10 py-4 bg-slate-50/50 hover:bg-white border border-slate-200 rounded-xl focus:bg-white focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-500 transition-all font-bold text-slate-800 outline-none cursor-pointer appearance-none text-sm shadow-sm focus:shadow-md"
                                    >
                            ...
                            <button
                                ...
                                className="bg-slate-900 hover:bg-indigo-600 text-white px-8 py-4 rounded-xl font-bold text-sm transition-all shadow-lg hover:shadow-indigo-500/30 hover:-translate-y-0.5 flex items-center justify-center gap-2 whitespace-nowrap disabled:opacity-70 disabled:cursor-not-allowed disabled:transform-none disabled:shadow-none"
                            >
                    ...
                    {loading && (
                        <div className="mt-8">
                            <PremiumLoader text="Analyzing Seating" subText="Cross-referencing room allocations..." />
                        </div>
                    )}
                ...
                <div>
                    <div className="flex items-center gap-3 mb-8">
                        ...
                        <h2 className="text-2xl font-black text-slate-900 tracking-tight">Active & Upcoming Exams</h2>
                    </div>
                    <PublishedExams />
                </div>
            </div>
            ...
            <div id="results-section">
                <AnimatePresence mode="wait">
                    {results && (
                        <div className="bg-[#f8f9fc] border-t border-slate-200 py-16">
                            <div className="container mx-auto px-4 md:px-6 max-w-6xl">
                                <div className="flex items-center justify-center gap-3 mb-12">
                                    <span className="h-px w-12 bg-slate-300"></span>
                                    <span className="text-slate-400 font-bold uppercase tracking-widest text-xs">Search Results</span>
                                    <span className="h-px w-12 bg-slate-300"></span>
                                </div>
                                <motion.div
                                    initial={{ opacity: 0, y: 20 }}
...
function StudentResultItem({ seat }) {
    ...
    useEffect(() => {
        const fetchLayout = async () => {
            ...
            try {
                const url = `${API_BASE_URL}/api/exams/${seat.ExamId}/rooms/${encodeURIComponent(seat.room_name)}/layout`;
...
function TicketCard({ seat, seatLabel }) {
    ...
    const getBatchDetails = () => {
        if (seat.Exam.batches) {
            let batches = seat.Exam.batches;
    ...
    return (
        <div className="filter drop-shadow-xl">
            <div className="flex flex-col md:flex-row bg-white rounded-3xl overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <div>
                            ...
                            <p className="text-indigo-300 font-medium text-sm">
                                {batch?.subject || (() => {
                                    let sub = seat.Exam.subjects;
                                    if (typeof sub === 'string') { try { sub = JSON.parse(sub); } catch (e) { sub = []; } }
                                    return Array.isArray(sub) ? sub[0] : '';
                                })()}
                            </p>
                        </div>
                    </div>
                ...
                <div className="flex-grow p-6 md:p-8 relative bg-white">
                    <div className="flex flex-col md:flex-row gap-6 md:gap-8 h-full">
                        <div className="flex-grow space-y-4 md:space-y-6">
                            ...
                            <div>
                                ...
                                <p className="text-xl md:text-xl font-black text-slate-900 tracking-tight break-all">{seat.student_reg}</p>
                            </div>
...
function TheaterView({ seat, layout, loading }) {
    ...
    return (
        <div className="flex flex-col items-center w-full max-w-full select-none">
            ...
            <div className="w-full relative group mb-8">
                ...
                <div className="overflow-x-auto pb-12 custom-scrollbar flex justify-center min-h-[300px]">
                    <div className="flex flex-col-reverse gap-3 items-center px-8 md:px-0 min-w-max mx-auto">
                        {/* Column Headers */}
                        <div className="flex flex-row-reverse gap-1 md:gap-3 items-center mb-1">
                            {/* Spacer for Row Label */}
                            <div className="w-4 md:w-6 text-center"></div>
                            {Array.from({ length: room.cols }).map((_, c) => {
                                const colNum = c + 1;
                                const isAisle = room.aisle_interval > 0 && (colNum % room.aisle_interval === 0) && (colNum !== room.cols);
                                return (
                                    <div key={c} className={`w-5 md:w-12 text-center text-[8px] md:text-sm font-black text-slate-300 ${isAisle ? 'ml-4 md:ml-8' : ''}`}>
                                        {colNum}
                                    </div>
                                );
                            })}
                        ...
                        {grid.map((row, ri) => (
                            <div key={ri} className="flex flex-row-reverse gap-1 md:gap-3 items-center">
                                {/* Row Label */}
                                <div className="w-4 md:w-6 text-center">
                                    <span className="text-[8px] md:text-xs font-black text-slate-300 uppercase">{String.fromCharCode(65 + ri)}</span>
                                </div>
                                ...
                                {row.map((cell, ci) => {
                                    ...
                                    // Base Styles
                                    // Mobile: w-5 h-5 (20px), Desktop: w-12 h-12 (48px)
                                    let baseClasses = "relative w-5 h-5 md:w-12 md:h-12 rounded md:rounded-lg flex items-center justify-center shrink-0 border md:border-2 transition-all duration-300 shadow-sm";
                                    let content = null;

                                    if (cell.isDisabled) {
                                        // Blocked / Disabled Seat
                                        baseClasses += " bg-slate-100 border-slate-200 opacity-60 cursor-not-allowed";
                                        content = <span className="text-slate-300 text-[8px] md:text-[10px] select-none">/</span>;
                                    }
                                    else if (cell.isMySeat) {
                                        // Yellow/Amber Theme for My Seat
                                        baseClasses += " bg-amber-100 border-amber-400 shadow-[0_0_15px_rgba(251,191,36,0.5)] z-20 scale-110";
                                        content = <span className="text-[8px] md:text-sm font-black text-amber-700">{seat.student_reg.slice(-3)}</span>;
                                    }
                                    else if (cell.occupied) {
                                        if (cell.batchStyle) {
                                            baseClasses += ` ${cell.batchStyle} opacity-90`;
                                        } else {
                                            // Green/Emerald for others (Standard)
                                            baseClasses += " bg-white border-emerald-500 hover:bg-emerald-50";
                                        }
                                        content = <span className="text-[6px] md:text-[10px] font-bold text-slate-400 opacity-60">{cell.occupied.student_reg.slice(-3)}</span>;
                                    }
                                    else {
                                        // Green/Emerald for Empty (Standard) - NO NUMBER
                                        baseClasses += " bg-white border-emerald-500 hover:bg-emerald-50 hover:shadow-md hover:scale-105";
                                        content = null; // Removed number for empty seats
                                    }
                                            ...
                                            {cell.isMySeat && (
                                                <div className="absolute -top-1 -right-1">
                                                    <span className="relative flex h-2 w-2 md:h-3 md:w-3">
                                                        ...
                                                        <span className="relative inline-flex rounded-full h-2 w-2 md:h-3 md:w-3 bg-amber-500 border md:border-2 border-white"></span>
                                                    </span>
                                                </div>
                                            )}
                                        </div>
            ...
            {/* Stage Indicatory (Moved to Bottom) */}
            <div className="w-full flex justify-center mb-8 px-4">
                {/* 3D Stage Effect */}
                <div className="w-full max-w-2xl text-center">
                    <div className="h-16 w-full bg-gradient-to-t from-blue-100/50 to-transparent transform perspective-[500px] rotate-x-12 border-b-4 border-blue-100 rounded-b-xl mb-2"></div>
                    <span className="text-xs font-black text-slate-300 uppercase tracking-[0.5em]">Stage / Screen Area</span>
                </div>
            </div>
            ...
            <div className="flex flex-wrap justify-center gap-6 mt-2 pb-6 border-t border-slate-100 pt-6 w-full">
                <div className="flex items-center gap-2">
                    <div className="w-5 h-5 rounded-md bg-amber-100 border-2 border-amber-400 shadow-sm flex items-center justify-center">
                        <span className="w-2 h-2 rounded-full bg-amber-500"></span>
                    </div>
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">My Seat</span>
                </div>
                <div className="flex items-center gap-2">
                    <div className="w-5 h-5 rounded-md bg-white border-2 border-emerald-500 flex items-center justify-center">
                        <span className="text-[8px] font-bold text-slate-400">123</span>
                    </div>
                    <span className="text-[10px] font-bold text-slate-500 uppercase tracking-wide">Occupied</span>
                </div>
                <div className="flex items-center gap-2">
                    <div className="w-5 h-5 rounded-md bg-white border-2 border-emerald-500"></div>
...
export default function StudentPage() {
    return (
        <Suspense fallback={<PremiumLoader text="Initializing Portal" subText="Establishing secure connection..." />}>
            <StudentContent />
        </Suspense>
    );
}
