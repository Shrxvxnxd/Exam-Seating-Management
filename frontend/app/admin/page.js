...
import { Calendar, Clock, MapPin, Users, Hash, LayoutGrid, Loader2, CheckCircle, AlertCircle, Plus, Trash2, Layers, AlertTriangle, Monitor, Settings, Link2, FileSpreadsheet, FileText, GraduationCap, Type, Pencil, Lock, LogOut, User, X, ShieldAlert, Activity, Eye } from 'lucide-react';
...
export default function AdminPage() {
    ...
    const canViewExams = hasPermission('create_exams') || hasPermission('edit_exams') || hasPermission('delete_exams') || hasPermission('archive_exams') || hasPermission('publish_exams') || hasPermission('download_attendance') || hasPermission('download_seating') || hasPermission('malpractice_entry') || userRole === 'view_only';
    const canCreateOrEdit = (editingExamId ? hasPermission('edit_exams') : hasPermission('create_exams')) && userRole !== 'view_only';
    ...
    const handleInfoChange = (e) => !isViewOnly && setBasicInfo({ ...basicInfo, [e.target.name]: e.target.value });
    ...
    const handleSeatClick = (roomIndex, r, c) => {
        ...
        if (isViewOnly) return; // Prevent clicking seats in View Only
    ...
    const handleEdit = (exam) => {
        ...
        const subjects = safeParse(exam.subjects);
        const batches = safeParse(exam.batches);
        const roomConfig = safeParse(exam.room_config);
        const excludedReg = safeParse(exam.excluded_reg);
        ...
        const globalExcluded = new Set(Array.isArray(excludedReg) ? excludedReg : []);
        ...
        if (Array.isArray(roomConfig) && roomConfig.length > 0) {
            const newRooms = roomConfig.map((r, i) => ({
                ...r,
    ...
    return (
        <div className="min-h-screen bg-slate-50 font-sans text-slate-900 pb-20">
            ...
            <div className={`sticky top-0 z-30 transition-all duration-300 ${stats.totalStudents > stats.totalEffectCapacity
                ...
                <div className="container mx-auto px-6 py-4 flex flex-wrap justify-between items-center gap-2 text-xs md:text-sm font-medium relative">
                    ...
                    <div className="hidden md:flex items-center gap-2">
                        ...
                        {(hasPermission('manage_admins') || userRole === 'view_only') && (
                            <button
                                onClick={() => setShowFacultyModal(true)}
                                className="bg-blue-600 hover:bg-blue-700 text-white px-3 py-1.5 rounded-md text-xs font-bold transition-colors shadow-sm flex items-center gap-1"
                            >
                                <span className="w-2 h-2 rounded-full bg-blue-300"></span>
                                {userRole === 'view_only' ? 'View Faculty' : 'Manage Faculty'}
                            </button>
                        )}
                        {(hasPermission('manage_admins') || userRole === 'view_only') && (
                            <button
                                onClick={() => { setShowStudentModal(true); }}
                                className="bg-indigo-600 hover:bg-indigo-700 text-white px-3 py-1.5 rounded-md text-xs font-bold transition-colors shadow-sm flex items-center gap-1"
                            >
                                <span className="w-2 h-2 rounded-full bg-indigo-300"></span>
                                {userRole === 'view_only' ? 'View Students' : 'Manage Students'}
                            </button>
                        )}
                        ...
                        {hasPermission('manage_maintenance') && (
                            <button
                                ...
                                className="bg-rose-50 hover:bg-rose-100 text-rose-700 px-3 py-1.5 rounded-md text-xs font-bold transition-colors shadow-sm flex items-center gap-1 border border-rose-200"
                            >
                        ...
                        {hasPermission('view_health') && (
                            <button
                                ...
                                className="bg-emerald-950 hover:bg-emerald-900 text-emerald-400 px-3 py-1.5 rounded-md text-xs font-bold transition-colors shadow-sm flex items-center gap-1 border border-emerald-900/50"
                            >
                    ...
                    <AnimatePresence>
                        {mobileMenuOpen && (
                            ...
                            >
                                ...
                                {hasPermission('manage_admins') && (
                                    <button
                                        onClick={() => { window.location.href = '/admin/maintenance'; setMobileMenuOpen(false); }}
                                        className="w-full text-left bg-rose-50 text-rose-700 px-4 py-3 rounded-xl text-sm font-bold shadow-sm flex items-center gap-3 active:scale-[0.98] transition-transform border border-rose-100"
                                    >
            ...
            <div className="container mx-auto p-4 lg:p-8 flex flex-col lg:grid lg:grid-cols-12 gap-6 lg:gap-8 mt-2">
                {/* --- Left Column: Form --- */}
                {(editingExamId ? (hasPermission('edit_exams') || isViewOnly) : hasPermission('create_exams')) ? (
                    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="lg:col-span-8 space-y-6">
                        <form onSubmit={handleSubmit} className="space-y-6">
                            ...
                            {
                                editingExamId && (
                                    <button type="button" onClick={resetForm} className="w-full mt-2 text-slate-400 text-sm font-bold hover:text-slate-600">{isViewOnly ? 'Close View' : 'Cancel Edit'}</button>
                                )
                            }
                        </form >
                    </motion.div >
            ...
            < AnimatePresence >
                {notification && (
                    ...
                    >
                        <div className={`px-6 py-4 rounded-2xl shadow-2xl flex items-center gap-3 border ${notification.type === 'success' ? 'bg-emerald-50 border-emerald-100 text-emerald-700' : notification.type === 'info' ? 'bg-blue-50 border-blue-100 text-blue-700' : 'bg-red-50 border-red-100 text-red-700'}`}>
                            {notification.type === 'success' ? <CheckCircle size={24} className="fill-current" /> : notification.type === 'info' ? <AlertCircle size={24} className="fill-current" /> : <AlertTriangle size={24} className="fill-current" />}
                            <div>
                                <h4 className="font-bold text-lg">{notification.type === 'success' ? 'Success' : notification.type === 'info' ? 'Info' : 'Error'}</h4>
            ...
            {showFacultyModal && <FacultyManagementModal isOpen={showFacultyModal} onClose={() => setShowFacultyModal(false)} />}
            {showStudentModal && <StudentManagementModal isOpen={showStudentModal} onClose={() => setShowStudentModal(false)} />}
            {showProfileModal && <ProfileModal isOpen={showProfileModal} onClose={() => setShowProfileModal(false)} />}
            {malpracticeExam && <MalpracticeModal isOpen={!!malpracticeExam} onClose={() => setMalpracticeExam(null)} exam={malpracticeExam} />}
            {malpracticeReportExam && <MalpracticeReportModal isOpen={!!malpracticeReportExam} onClose={() => setMalpracticeReportExam(null)} exam={malpracticeReportExam} />}
...
function ExamCard({ exam, handlePublish, handleDelete, handleEdit, handleArchive, handleMalpractice, handleMalpracticeReport, setNotification, hasPermission, isViewOnly }) {
    ...
    // Safely parse potentially stringified data
    // We use these local variables instead of exam.* to prevent crashes
    const subjects = Array.isArray(safeParse(exam.subjects)) ? safeParse(exam.subjects) : [];
    const batches = Array.isArray(safeParse(exam.batches)) ? safeParse(exam.batches) : [];
    ...
    const handleStudentView = () => {
        ...
        const link = `/student?exam=${encodeURIComponent(exam.name)}&date=${exam.date}`;
        // Premium artificial delay for "loading" feel as requested
        setTimeout(() => {
        ...
        <div className="bg-white border border-slate-200 shadow-sm p-5 rounded-2xl relative overflow-hidden group transition-all">
            <div className="flex flex-col-reverse gap-4 mb-4">
                ...
                <div className="flex flex-wrap gap-2 items-center self-end md:self-auto">
                    ...
                    {!exam.archived && isViewOnly && (
                        <button onClick={() => {
                            handleEdit(exam);
                            if (setNotification) setNotification({ type: 'info', message: 'View Mode: You can view details but cannot edit.' });
                            setTimeout(() => setNotification && setNotification(null), 3000);
                        }} className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition border border-transparent hover:border-indigo-100" title="View Details">
                            <Eye size={14} />
                        </button>
                    )}
                    {(exam.status === 'published' || exam.archived) && (
                        <div className="flex gap-1">
                            ...
                            <button onClick={handleStudentView} className="p-1.5 text-slate-400 hover:text-emerald-600 hover:bg-emerald-50 rounded-lg transition border border-transparent hover:border-emerald-100 relative" title="Student View">
                                {isNavigating ? <Loader2 size={14} className="animate-spin text-emerald-500" /> : <Monitor size={14} />}
                            </button>
                        </div>
                    )}
