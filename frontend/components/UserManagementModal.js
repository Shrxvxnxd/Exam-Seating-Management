...
const PERMISSIONS_LIST = [
    ...
    { id: 'malpractice_entry', label: 'Malpractice Entry' }, // Restricted for View Only
    { id: 'view_health', label: 'View System Health' },
    { id: 'manage_maintenance', label: 'Manage Maintenance' }
];

const VIEW_ONLY_PERMISSIONS = ['view_health', 'download_attendance', 'download_seating']; // Default allowed for View Only
    ...
    const handleCreate = async (e) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                if (newUser.role === 'view_only') setSelectedPermissions(VIEW_ONLY_PERMISSIONS);
                else setSelectedPermissions([]); // Reset defaults
                                        ...
                                        <div className="flex justify-between items-start mb-2">
                                            <div className="font-bold text-slate-700">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {user.role === 'view_only' && (
                                                        <span className="text-[10px] bg-cyan-100 text-cyan-700 px-1.5 py-0.5 rounded border border-cyan-200 font-bold tracking-wider">VIEW ONLY</span>
                                                    )}
                                                </div>
                    ...
                    <div className={`flex-1 p-4 md:p-6 overflow-y-auto relative bg-white ${mobileTab === 'create' ? 'block' : 'hidden'} md:block`}>
                        ...
                        <form onSubmit={handleCreate} className="space-y-5">
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        <label className="flex items-center cursor-pointer">
                                            <input
                                                ...
                                                checked={newUser.role === 'view_only'}
                                                onChange={() => {
                                                    setNewUser({ ...newUser, role: 'view_only' });
                                                    setSelectedPermissions(VIEW_ONLY_PERMISSIONS); // Auto-select safe permissions
                                                }}
                                            />
                                            <span className="ml-2 text-sm font-semibold text-cyan-700">View Only</span>
                                        </label>
                                        <label className="flex items-center cursor-pointer">
                                            <input
                                                type="radio"
                            ...
                            {newUser.role === 'main_admin' ? (
                                ...
                                <div className="p-4 bg-cyan-50 border border-cyan-100 rounded-lg text-center space-y-3">
                                    <div className="text-sm font-bold text-cyan-800 flex items-center justify-center gap-2">
                                        <Shield size={16} /> Restricted View Access
                                    </div>
                                    <p className="text-xs text-cyan-600">
                                        "View Only" admins can only access read-only data (Attendance, Seating, Health).
                                        <br />Write operations are strictly blocked by the server.
                                    </p>
                                    <div className="grid grid-cols-2 gap-2 text-left opacity-80">
                                        {VIEW_ONLY_PERMISSIONS.map(pId => {
                                            const p = PERMISSIONS_LIST.find(x => x.id === pId);
                                            return (
                                                <div key={pId} className="flex items-center gap-2 text-[10px] text-cyan-700 font-bold bg-white px-2 py-1 rounded border border-cyan-100">
                                                    <CheckCircle size={10} className="text-cyan-500" /> {p ? p.label : pId}
                                                </div>
                                            );
                                        })}
                                    </div>
